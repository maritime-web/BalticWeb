/**
 * Downloaded from https://github.com/springmeyer/arc.js
 * Version 0.1.0
 */
var D2R = Math.PI / 180;
var R2D = 180 / Math.PI;
...
var GreatCircle = function(start, end, properties) {
    ...
    var z = Math.pow(Math.sin(h / 2.0), 2) + Math.cos(this.start.y) * Math.cos(this.end.y)
    ...
    if (bHasBigDiff && dfMaxSmallDiffLong < dfDateLineOffset) {
        ...
        for ( var k = 0; k < first_pass.length; ++k) {
            ...
            if (k > 0 && Math.abs(dfX0 - first_pass[k - 1][0]) > dfDiffSpace) {
                ...
                if (dfX1 > -180 && dfX1 < dfRightBorderX && dfX2 == 180 && k + 1 < first_pass.length
                        && first_pass[k - 1][0] > -180 && first_pass[k - 1][0] < dfRightBorderX) {
                    poNewLS.push([ -180, first_pass[k][1] ]);
                    k++;
                    poNewLS.push([ first_pass[k][0], first_pass[k][1] ]);
                    continue;
                } else if (dfX1 > dfLeftBorderX && dfX1 < 180 && dfX2 == -180 && k + 1 < first_pass.length
