...
import static dk.dma.embryo.dataformats.model.ForecastMetaDataKeys.*;
...
public class ForecastMetaData {
    ...
    public ForecastMetaData withJsonSize(int jsonSize) {
        if (jsonSize <= 0) throw new IllegalArgumentException("Json size must be greater than zero");

        return with(createWith(JSON_SIZE.getKeyName(), jsonSize));
    }

    public ForecastMetaData withTimestamp(long timestamp) {
        if (timestamp <= 0) throw new IllegalArgumentException("Timestamp must be greater than zero");

        return with(createWith(TIMESTAMP.getKeyName(), timestamp));
    }

    public ForecastMetaData withOriginalFileName(String fileName) {
        if (StringUtils.isBlank(fileName)) throw new IllegalArgumentException("File name must contain at least one character");
