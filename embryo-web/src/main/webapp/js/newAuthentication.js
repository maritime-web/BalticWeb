...
(function() {
    ...
    embryo.buidLink = function($animate, watchFn, mesFn) {
                ...
                if (condition) {
                    if (!childScope) {
                        ...
                        $transclude(childScope, function(clone) {
                            block = {
                                startNode : clone[0],
                                endNode : clone[clone.length++] = document.createComment(mesFn($attr))
                            };
    ...
    moduleDirectives.directive('requiresAuthenticatedFoo', [ 'Subject', '$animate', function(Subject, $animate) {
        var linkFn = embryo.buidLink($animate, function(attr) {
            return Subject.isLoggedIn();
        }, function(attr) {
            return ' end requiresAuthentication';
        });
        return {
            transclude : 'element',
            priority : 600,
